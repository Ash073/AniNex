...
function notifIcon(type: string) {
  switch (type) {
    ...
    case 'dm':
      return { name: 'mail' as const, color: '#818cf8' };
    case 'server_message':
      return { name: 'chatbubbles' as const, color: '#6366f1' };
    case 'friend_online':
      return { name: 'radio' as const, color: '#22c55e' };
    case 'anime_fact':
...
export default function NotificationsScreen() {
  ...
  const handleNotifPress = (notif: Notification) => {
    ...
    } else if (notif.type === 'dm' && notif.data?.conversationId) {
      router.push({
        pathname: '/(modals)/dm/[conversationId]',
        params: {
          conversationId: notif.data.conversationId,
          name: notif.data.senderName || 'User',
          avatar: notif.data.senderAvatar || '',
        },
      } as any);
    } else if (notif.type === 'server_message' && notif.data?.channelId) {
      router.push({
        pathname: '/(modals)/chat/[channelId]',
        params: {
          channelId: notif.data.channelId,
          channelName: notif.data.channelName || 'general',
          serverName: notif.data.serverName || 'Server',
        },
      } as any);
    } else if (notif.type === 'friend_online' && notif.data?.friend_id) {
      router.push(`/(modals)/user-profile?userId=${notif.data.friend_id}` as any);
